...
SCRIPT_PATH = os.path.join(BASE_PATH, 'prepare_data', 'manifold40.mlx')
DATASET_PATH = os.path.join(DATA_PATH, 'Manifold40_Processed')
...
def collect_labels():
    PATH = os.path.join(DATA_PATH, 'Manifold40')
...
def build():
    ...
    for i in range(len(labels)):
        PATH = os.path.join(DATA_PATH,'Manifold40',labels[i])
...
if __name__ == '__main__':
    if not os.path.exists(os.path.join(DATA_PATH, 'Manifold40')):
