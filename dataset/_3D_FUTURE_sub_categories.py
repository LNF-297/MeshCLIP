...
# get classnames for fewshot
def get_classnames_fs():
    from data._cache.categories import _CATEGORIES_3D as categories
    classname = []
    for dir in categories:
        # omit some classes as explained in paper
        if dir['id'] == 40 or dir['id'] == 42:
            continue
        classname.append(dir['category'])
    return classname

# get classnames for zero shot
def get_classnames_zs():
    from data._cache.categories import _CATEGORIES_3D as categories
    classname = []
    for dir in categories:
        classname.append(dir['category'])
...
class dataset_index(Dataset):
    def __init__(self, PATH):
        ...
        name_id_index = {}
        for dir in categories:
            if dir['category'] == 'Floor Lamp':
                name_id_index[dir['category']] = 39
            else:
        ...
        for cat in js_index:
            ...
            if not classname in name_id_index.keys():
                continue
            if classname == 'U-shaped Sofa' or classname == 'Wall Lamp':
                continue
...
def dataset_fs(PATH, num_shot):
    fullset_index = dataset_index(PATH)
    size = len(fullset_index)
    train_size = int(size*2/3)
    test_size = size - train_size
    trainset, testset = random_split(fullset_index, [train_size, test_size])
#    trainset = fewshot_set(trainset, num_shot)
    testset = full_set(testset)
    trainset = full_set(trainset)
    return trainset, testset

# Dataset for zeroshot
